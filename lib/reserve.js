...
let reserve = async () => i18n['reserve_help']

reserve.list = async args => {
  if (args.length < 2) {
    let {body} = await request.get(baseURL + '/reservation/')
    return view.showList(body.reservations)
  }
  else {
    let [month, date] = args
    let {body} = await request.get(baseURL + '/reservation/' + month + '/' + date)
    return view.showOneDayList(body.reservations, month, date)
  }
}

reserve.all = async args => {
  let {body} = await request.get(baseURL + '/reservation/')
  return view.showFullList(body.reservations)
}
...
reserve.delete = async (args, userId) => {
