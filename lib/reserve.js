...
const {baseURL, places} = require('../config')
...
reserve.new = async (args, userId, userLang) => {
  ...
  // Convert room number back to number representation
  let isPlaceHit = false
  for (let index in places) {
    if (place == places[index]) {
      place = index
      isPlaceHit = true
      break
    }
  ...
  if (!isPlaceHit) {
    if (place != Number(place)) {
      return i18n['reserve_place_not_recognize'].format(place)
    }
  }
